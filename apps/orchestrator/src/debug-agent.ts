...
function buildSingleTurnSystemPrompt(): string {
  return [
    ...
    "- draft_outlook_mail: create a brand new mail draft shell, then use update_outlook_draft to set a polished subject/body when needed. to and cc may contain one or more individual addresses or group/distribution addresses.",
...
function buildLoopSystemPrompt(): string {
  return [
    ...
    "- draft_outlook_mail: create a new mail draft shell, then use update_outlook_draft to set the final subject and body. to and cc may contain one or more individual addresses or group/distribution addresses.",
    ...
    "For mail tasks, prefer search_outlook_mail -> read_outlook_mail/read_outlook_conversation -> draft/reply -> preview_outlook_draft -> finish_task for approval, and only then send_outlook_mail after explicit approval. After sending, use await_email_reply when the current agent should wait in-line for a reply, or watch_email_reply when a persisted workflow should resume later via external event.",
    "expected_transition must be one of: READY, RUNNING, DRAFT_READY, APPROVAL_REQUIRED, WAITING_EMAIL, WAITING_CHAT, WAITING_HUMAN, WAITING_SYSTEM, COMPLETED, FAILED, ESCALATED.",
