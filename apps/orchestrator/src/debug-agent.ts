...
function buildSingleTurnSystemPrompt(): string {
  return [
    ...
    "Mail tool semantics:",
    "- search_outlook_mail: search mailboxes and return message list candidates.",
    "- read_outlook_mail: read one concrete mail after search, by entry_id or conversation_id.",
    "- read_outlook_conversation: read the mail thread when context from multiple messages matters.",
    "- draft_outlook_mail: create a brand new mail draft shell, then use update_outlook_draft to set a polished subject/body when needed.",
    "- reply_outlook_mail: create a reply draft in the existing thread.",
    "- update_outlook_draft: revise draft recipients, subject, or body.",
    ...
    "- watch_email_reply: register a reply watcher after sending when the task requires waiting for a reply. Use conversation_id when known, expected_from for sender filtering, and keyword_contains for subject/body keyword filtering.",
...
function buildLoopSystemPrompt(): string {
  return [
    ...
    "- search_outlook_mail: search Outlook mailboxes and return candidate messages to inspect further.",
    "- read_outlook_mail: read one specific mail in detail after search.",
    "- read_outlook_conversation: read a whole mail thread when you need context across replies.",
    "- draft_outlook_mail: create a new mail draft shell, then use update_outlook_draft to set the final subject and body.",
    "- reply_outlook_mail: create a reply draft tied to an existing message or conversation.",
    "- update_outlook_draft: revise a draft after you learn new information or get user feedback.",
    ...
    "- watch_email_reply: register a watcher after sending when you must resume later when a reply arrives. Use conversation_id when known, expected_from for sender filtering, and keyword_contains for subject/body keyword filtering.",
    ...
    "For mail tasks, prefer search_outlook_mail -> read_outlook_mail/read_outlook_conversation -> draft/reply -> preview_outlook_draft -> finish_task for approval, and only then send_outlook_mail after explicit approval.",
    "expected_transition must be one of: READY, RUNNING, DRAFT_READY, APPROVAL_REQUIRED, WAITING_EMAIL, WAITING_CHAT, WAITING_HUMAN, WAITING_SYSTEM, COMPLETED, FAILED, ESCALATED.",
