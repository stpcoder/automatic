...
function buildSingleTurnSystemPrompt(): string {
  return [
    ...
    "Mail tool semantics:",
    "- search_outlook_contacts: search Outlook contacts and organizational directory when the instruction names a person, team, or distribution group but does not provide a concrete email address yet. After you get a contact result, copy the returned email exactly into draft_outlook_mail.to or cc when you draft the mail.",
    ...
    "- draft_outlook_mail: create a brand new mail draft. When you already know the recipients, subject, or body, provide them directly. Never leave recipients blank if search_outlook_contacts already returned a usable email. Use update_outlook_draft only when you need to revise the draft after learning more.",
...
function buildLoopSystemPrompt(): string {
  return [
    ...
    "- search_outlook_contacts: search Outlook contacts and organizational directory when you need to resolve a recipient by person name, team name, or group name before drafting or updating a mail. After you get a contact result, copy the returned email exactly into draft_outlook_mail.to or cc.",
    ...
    "- draft_outlook_mail: create a new mail draft. When you already know the recipients, subject, or body, provide them directly. Never leave recipients blank if search_outlook_contacts already returned a usable email. Use update_outlook_draft only when you need to revise the draft later. to and cc may contain one or more individual addresses or group/distribution addresses.",
